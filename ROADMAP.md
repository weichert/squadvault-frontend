...
| W.1 | A/V Room **Increment 1**: fail-closed room, photo ingest, five-kind provenance tagging, vacuous-tag rejection, correction-by-supersession, ratification, withdrawal, honest gaps; video present-but-not-playable (deferred in Inc 1 - playback later SHIPPED via D-W1-A `478a034`; see the discharge ledger below) | Inc 1 done; video playback later SHIPPED (D-W1-A `478a034`); member-testimony display still deferred | `c21e858`, `df79a4f`, `c284053`, `7eee29d`, `65da2e6` |
...
| CO.1 | **Coach Office - Phase 1 static interactive shell** (new product surface, spec `docs/coach_office/final_spec_package_v1`; brief `_observations/session_brief_2026_06_30_coach_office_phase1_static_shell.md`; D-1 route + D-4 placeholders founder-locked 2026-06-30): new route `/league/[id]/coach-office/[coachId]` renders a neutral placeholder scene with manifest-driven hotspots (geometry in `lib/coach-office/hotspots.ts`, transcribed verbatim from the spec hotspot manifest; **no coordinate literal in components** - they consume the passed-in HotspotMap), labeled placeholder modals (accessible dialog: focus-trap, Escape/backdrop close, focus restore), and a mobile hotspot-list fallback (>=44px targets). `coachId` echoed as a placeholder identifier, NOT resolved to franchise data; only DB call is `getLeague`. No league/coach/team/joke literals (data-driven, charter section 2); banned-literal grep clean. type-check green; production build green at CI parity (Node 24, NODE_ENV=production - note local `.env.local` sets NODE_ENV=development which trips the Next 14 `<Html>` prerender bug on the error pages; CI unaffected). **OUT OF SCOPE (deferred to later phases):** DB/migrations, coach-office-profile loader, Trophy/Ring resolvers (P2), visitor filtering / relationship-aware rendering (P3), board messaging (P4), media/photo frames (P5), easter eggs (P6), cutouts (P7), QA/no-hard-coding gate (P8), real artwork, nav-tab integration. | Done | `fab143b` (#46) |
| CO.2 | **Coach Office - Phase 2 owner personalization** (brief `_observations/session_brief_2026_06_30_coach_office_phase2_owner_personalization.md`; D-2 derive-only + D-5 Weichert founder-locked 2026-06-30; stacked on CO.1): the office renders the coach's real team nameplate resolved from the `franchises` row by `canonical_franchise_id` (D-2: **derived, no `coach_office_profiles` table, no migration**), and the Trophy Case + Ring Box hotspots open real content DERIVED off the league championship record (reuses `trophy-room.ts` `loadChampionshipPackage`, filtered to the coach's franchise; era-correct; never invented). Empty state for a titleless coach; unknown coachId 404s. `resolveCoachOfficeProfile` + `resolveCoachChampionships` (reads only); `office-shell`/`hotspot-modal` made content-aware (per-hotspot body via the RSC slot pattern, placeholder fallback intact). Board / Framed Photos / Cardboard Cutout stay placeholders. Acceptance fixtures (data, not code): league `70985`, coach `0005` (Weichert's Warmongers) = 1 ring (2024). No fixtures/league/coach literals in code (banned-literal grep clean). type-check green; production build green at CI parity (Node 24, NODE_ENV=production). **OUT OF SCOPE:** Phase 2b (held Live Records + Season Awards in the Trophy Case), visitor filtering (P3), board (P4), media (P5), eggs (P6), cutouts (P7), QA gate (P8), artwork, nav integration, the profile table/migration. | Done | `c7d3048` (#47) |
| CO.2b | **Coach Office - Phase 2b Trophy Case awards** (stacked on CO.2; derive-only per D-2): the Trophy Case gains a "Records held" section - the traveling/annual/permanent records the coach CURRENTLY holds, derived off the Trophy Room read-models (`loadLiveRecords` + `loadSeasonAwards` annual/permanentCards) filtered to the coach's franchise uuid (derived holder C1, era-correct, never invented). `resolveCoachHeldRecords`; `TrophyCaseView` renders Championships + Records-held sections (empty state only when both empty); Ring Box unchanged. Excluded (deferred): permanent multi-lists (name-keyed) + player/auction awards #13-23 (engine-canonical-code keyed). Verified vs real DB: Weichert (`0005`) holds The Bridesmaid Bouquet (runner-up 2025), no Live Records / permanent cards. No fixtures/league/coach literals in code (grep clean). type-check green; production build green at CI parity (Node 24, NODE_ENV=production). **OUT OF SCOPE:** visitor filtering (P3), board (P4), media (P5), eggs (P6), cutouts (P7), QA gate (P8), artwork, nav integration. | Done | `0feb832` (#49) |
| RS.1 | Root splash: authed redirect + anonymous clubhouse landing | Done | `2c91f8b` (PR #50) |
| CO.3 | **Coach Office - Phase 3 relationship-aware viewer context foundation** (brief authored 2026-07-01 against HEAD `72acad4`; memo `_observations/OBSERVATIONS_2026_07_01_COACH_OFFICE_PHASE3_VIEWER_CONTEXT.md`; stacked on CO.2b; two prior mislabeled briefs returned as stale-git-wins before any code, see memo): the smallest safe layer answering "who is viewing this office, and what is their relationship to the owner?" - and NOT yet "what relationship-specific content to show." `src/lib/coach-office/viewer-context.ts` (new): taxonomy `OWNER | COMMISSIONER | LEAGUE_MATE | PUBLIC_OR_UNKNOWN`; **pure deterministic** `classifyCoachOfficeViewerContext` (precedence OWNER->COMMISSIONER->LEAGUE_MATE->PUBLIC_OR_UNKNOWN, owner-viewing-own-office wins over commissioner) + thin async `resolveCoachOfficeViewerContext` whose only I/O is ONE read-only `franchises` lookup (`member_user_id`->`canonical_franchise_id`; anonymous skips it). All cross-module imports are `import type` (classifier provable with no DB/Next). Conservative capability booleans (`canViewPublicOffice=true`, `canViewOwnerOnlySurface` iff OWNER, `canViewRelationshipSurface` iff not PUBLIC_OR_UNKNOWN) are **ADVISORY only - no consumer gates content on them**. Threaded page->`OfficeShell`->`HotspotModal` (neutral debug-safe `data-viewer-relationship` on the dialog root); **no content filtered, no visual branching, Trophy Case / Ring Box behavior preserved.** No new table/migration, no board/media/eggs/cutouts, no consent workflow. No league/coach/team/joke literals (grep clean, 5/5 files). Proof `scripts/proof_coach_office_viewer_context.ts` **40/40** (truth table + precedence + conservative default + advisory booleans). type-check green; production build green at CI parity (Node 24, NODE_ENV=production); governance **154/0** (no regression). **Deviation:** `lint` not run - repo has no ESLint config and CI runs type-check+build only (consistent with CO.1/CO.2/CO.2b); no config scaffolded (unapproved scope). **OUT OF SCOPE:** content filtering, board (P4), media (P5), eggs (P6), cutouts (P7), QA gate (P8), artwork, nav integration. | Done | `fce58d2` (#48) |
| MW.1 | **Middleware honesty + metadataBase fix** (rulings R1/R2 ratified 2026-07-04; brief `_observations/session_brief_2026_07_04_middleware_honesty_metadatabase.md`; memo `_observations/OBSERVATIONS_2026_07_04_MIDDLEWARE_HONESTY_METADATABASE.md`; follows findings memo `2af142d`): makes the access-control code match the ratified design. New **registered** `src/middleware.ts` (root-level `middleware.ts` was never picked up under the `src/` convention - the CO.3 finding) as a SECOND layer behind the primary page-level self-gates; `config.matcher` lists ONLY the gated set (`office`/`vault`/`history`/`consent`/`av-room`/`approve` + `/admin/*`), so R1-public ceremonial surfaces (league home, coach-office, members, trophy-room, archive) stay world-visible and unmatched. Reuses the `@supabase/ssr` session-refresh pattern verbatim; anon bounce `/auth/login?redirect=<path>` at parity with the page gates; authed non-admin on `/admin/*` -> plain 403. Dead root file deleted; `layout.tsx` metadataBase fallback moved off the parked `squadvault.com` to `https://squadvault.vercel.app`. Registration proven (`ƒ Middleware` build line); route trace via Next's `path-to-regexp` (all R1-public UNMATCHED incl. `coach-office`, all gated MATCHED); page gates untouched. Inheritance rule recorded: future consent-scoped content on a public route needs BOTH a page gate AND a matcher entry. **Owed founder step:** set `NEXT_PUBLIC_APP_URL=https://squadvault.vercel.app` in Vercel. type-check + build green. **OUT OF SCOPE:** gating any R1-public surface, page-gate changes, auth flow, the `/auth/login` prerender finding, buying `squadvault.com`. | Done | `0662f5b` (#56) |
| QA.1 | Vitest auth harness: safeRedirectPath + resolveAuthSession | Done | `6b31abf` (#58) |
| CO-SPEC | Coach Office room spec v1 formally ratified (CO-R1..R4) - memo `docs/coach_office/RATIFICATION_MEMO_2026_07_04.md`; existing `docs/coach_office/final_spec_package_v1/` canonical (no re-copy/rename) | Done | `4d789b7` (#60) |
| W.2 | **Clubhouse Cluster 1 - the room** (DECIDE brief `_observations/session_brief_2026_07_05_w2_clubhouse_cluster1.md`; gates G1/G2/G3): navigable illustrated Clubhouse at `/league/[id]/clubhouse` - the locked Tahoe master (webp) as a full-bleed layered room; manifest-driven hotspots (`public/clubhouse/hotspots.json`, geometry never in components) over ten objects routing to existing surfaces (trophy-room/av-room/vault/office; gates enforced at destinations, unchanged), dignified pending modals (corkboard, answering machine), or inert fixtures; input-driven parallax on plate-backed objects with static reduced-motion fallback; banner text data-driven from getLeague ("<name> - Est. <founding_year>", never baked); additive CLUBHOUSE nav (first public tab). Room-agnostic `RoomScene`/`RoomModal` (Coach Office room reuses, no fork). Plates shipped near-lossless webp (alpha bit-exact; 17.3MB->1.7MB; runtime ~2.0MB); pristine PNGs stay in the asset folder w/ MANIFEST provenance. Excluded pending re-render (checkerboard-defect plates - hotspots draw on master): trophy/phone/answering. tsc/build green (Node 24, NODE_ENV=production), vitest 50/50 (+ manifest schema/order/route-trace test). No engine changes. As shipped (`b63e8da`): master-only v1 (object silhouette glow + parallax deferred to a registered-cutout follow-on, "Active Objects"); SEVEN hotspots, not ten (the four inert character objects - phone/guitar/boombox/hearth - were dropped so every hotspot rewards a click; the laptop was added as a route to league home); banner is a curved SVG textPath on the cloth (smile + rotation) with a room-light gradient + fabric integration (cloth luminance masked onto the glyphs), full line reads at rest on mobile; vitest 47/47. | Done | `b63e8da` |
| W.5 | **Trophy Room "Living Room" pivot** (G1/G2 Fable rulings 2026-07-07, relayed; memo `_observations/OBSERVATIONS_2026_07_07_TROPHY_ROOM_LIVING_ROOM_PIVOT.md`; stacked on trophy-hall v2 `aaef5bb`/`53b448f`): the baked LOCKED master (tr_master_web, CO-R4-clean) becomes the navigational room - six case click zones + runtime labels over painted plaques, League Trophy + reigning champion on the plinth (D-PLINTH; full champion roll in the detail; the Ring is a Championship-case object). Case click -> frontal CASE VIEW (tr_case_frontal_web) as a RoomModal overlay, never a route (D-CASE-NAV): 5 marquee bands held-first + "See all N" reusing the shipped category grid (D-BANDS - no hidden facts, pinned as a strict-prefix property); numeral-free category note on the lower panel (D-LOWER). Naming sweep Hall->Room, zero route changes; nav tab fronts the room, full record one tap deeper (D-NAV); the /trophy-room reachability pin RE-HOMED to the page-level link (G2 ruling 3). Manifest v3 owns all geometry (zones-only cases, plinth, case_view 5-band block; per-shelf v2 geometry superseded). Pure seam `case-view-bands.ts` (marquee/bands/plinth/placard honesty; placeOnBands throws on overflow per G2 ruling 2). Tests-first 30-test package (G2-cleared; RED = exactly the 9 owed pins) now green: vitest 195/195, tsc clean, prod build at CI parity. Local eyeball limited to a geometry harness (local .env.local is placeholder-creds; league routes 404 locally) - **G3 founder eyeball on the deployed preview gates the push**. trophy-preview retarget OUT (G2 ruling 4; registered follow-on). | Done pending G3 | `603b469` |
| CO-ROOM-V1 | Coach Office Room v1 — shell + resolvers | Done | `936a8ee` (#66) |
| TROPHY-HALL-V1 | **Trophy Hall — illustrated room v1** (W.5 dual-layer; brief `36f8ca3`, gates G1/G2/G3 cleared): SquadVault's fourth illustrated room — a pure presentation of the shipped W.5 trophy fact layer. SHIPPED: the founder-confirmed Tahoe master render + three illustrated plates (Hammer/Benchwarmer/Clairvoyant, true-alpha webp), the group-taxonomy card layout, the viewer-relative reflective highlight ("your hardware" — a member's own held trophies, identity reconciled across both id spaces), and **the provenance toggle** (first room to build it — reveals the shipped fact layer inline; unit-proven faithful receipt + object↔receipt alignment). Reuses the room-agnostic `RoomScene` via an additive `objects` overlay (no fork); pure node-env seams (`viewer-holdings`, `provenance-receipt`) hold all correctness. Reads generated awards `award_id 3/6/7/9` **read-only** (existing awards byte-identical); generated-award numeric values omitted (no confirmed unit — silence over a misread number). Clubhouse trophy-case hotspot reroutes here; the `/trophy-room` fact page persists. **~30 awards render as text-fallback pending art.** Zero engine changes; tsc clean, vitest 120/120. **Open: cases→category→trophy-detail interaction depth is a ratified v2 increment (brief incoming); the four generated awards light on prod only after the founder's seed-004 apply; Oracle awaits sundial art.** | Done | `8380a78` (#77) |
| SIGNED-IN-INDICATOR | **Signed-in toolbar chip** (brief `1a9fced`, gates G1/G2/G3 cleared): a slim fixed top-right chip on every nav-bearing league page (both presentations) showing auth state — signed in → the franchise display name; signed-in-without-franchise → honest "Signed in" / "(commissioner)" fallback (never fabricated or blank); anonymous → a "Sign in" affordance. Status DISPLAY only: auth resolves SERVER-SIDE in the league layout (`getViewer` consumed unchanged) → serializable state → additive optional `indicator?` nav prop (byte-identical when absent, no fork); the presentational chip reads no session, so there is nothing to record client-side (no analytics). One additive read (`franchises.owner_display_name`, the CO.3 pattern; `viewer-context.ts` unmodified). Anonymous redirect `?redirect=/league/{id}` validated through the shipped `safeRedirectPath` (same-origin only; absolute + protocol-relative neutralized). Correctness in pure node-env seams (honest-fallback resolution + redirect validation). G3 verified on preview (anonymous → "Sign in" → sign-in flow reached). Zero engine changes; tsc clean, vitest 134/134. | Done | `8a1bcec` (#81) |
| NEW-MEMBER-LANDING | **Post-login + post-consent land in the Clubhouse** (brief `b012b15`, auth/consent-critical; gates G1/G2/G3 + a G3 finding + fix): a member's journey ends in the illustrated Clubhouse, not the data ledger. **Change A** (post-login): the authed-root redirect + splash sign-in link target `/league/{id}/clubhouse` (`page.tsx`, two literals; the `?redirect=`+`safeRedirectPath` mechanism untouched). **Change B** (post-consent): a **coercion-free** "Enter the Clubhouse →" affordance in the consent panel — rendered unconditionally (grant nothing and still enter, W.6 default-no); `act()`/`router.refresh()` unchanged (recording a grant never navigates away). **Change A-2** (G3-finding fix): the signed-in chip's anonymous "Sign in" redirect — a post-login entry point that postdated the original trace and still targeted the bare data home — repointed to the Clubhouse (`layout.tsx` `buildSignInHref(clubhouseHref(id), origin)`). Canonical-id throughout via the new pure `clubhouseHref(id)` seam (`id` = params canonical; a UUID `league.id` path 404s); consent-first preserved (invite→`/consent` untouched); gated-page login redirects untouched (return-to-denied-page); data home reachable via the Community nav tab. G3 verified live: consent door coercion-free with zero grants, no bounce on toggle, no 404, redirect proven in-flight. Zero engine changes; tsc clean, vitest 150/150. **(1) Named post-merge verification (OPEN):** the email→Clubhouse hop is PROD-ONLY by Supabase Site-URL architecture (magic-link redirects resolve to the project Site URL, not a preview origin); the founder confirms it live on production once this deploys — if it fails there, that is a NEW finding against main (append-only). **(2) A-2 lesson (register):** the chip entry point postdated the trace; the literal tests passed while the lived path bypassed the change, caught only by the founder eyeball — write BEHAVIORAL-DESTINATION tests (where does the flow actually land), not just literal-edit tests, from here on. **(3) Non-blocking notes:** the "Commissioner Sign In" page label should read as member sign-in (one-word copy fix; pre-Draft-Weekend sweep); the Supabase email rate limit (dashboard) should be raised modestly before nine members sign in on Draft Weekend. | Done | `0b86c69` (#84) |
