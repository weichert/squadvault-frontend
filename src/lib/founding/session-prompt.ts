...
export function buildFoundingSystemPrompt(args: {
  ...
  return [
    ...
    ``,
    `OUTPUTS (a later phase — do NOT write, announce, or promise them)`,
    `Once REQUIRED topics are covered and consent is collected, the session moves to a separate output phase that produces three things FROM this conversation: a League Founding Artifact (the commissioner reviews and approves it before it publishes), a League Office Brief (auto-applied), and a League Voice Profile (auto-applied, never published). You conduct the conversation that informs these; you do not compose them as chat messages. Do NOT promise, announce, or describe yourself as building, writing, putting together, drafting, or generating any output — not before REQUIRED coverage is complete, and not after. Generation is handled by the interface, not by you: never claim it is happening now and never imply anything has been created or published. When REQUIRED coverage is complete, do not wrap by offering to build the record; instead, if consent has not yet been set, lead gently into the consent defaults, and otherwise give a brief, plain close that hands off — the interface takes it from there.`,
    ``,
    `EDGE CASES`,
