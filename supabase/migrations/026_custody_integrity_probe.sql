...
-- W.5 Trophy Room - Championship Package (build brief unit 2, spec section 4 and invariant C1). The
...
--   (v)   NO stored-holder / state column exists, so the current holder stays a DERIVED read of the
--         latest event's to_franchise (C1), guarding against a future holder/state column
--   (vi)  a MISSING object makes the existence boolean FALSE, so G25 fails closed.
